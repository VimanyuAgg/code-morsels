...
def dijkstra(start_node, end_node):
    if not start_node or not end_node:
        return None
    #     pq = [[node, index ,float("inf")] for node, index in enumerate(graph.nodes) if node != start_node]

    pq = []
    counter = 0
    pq.append([0, counter, start_node])
    counter += 1
    heapq.heapify(pq)
    visited = set()
    ...
    while pq:
        ...
        for child in node.edges:
            if child not in visited:
                edge_cost = d + child.distance
                heapq.heappush(pq, [edge_cost, counter, child.node])
                counter += 1
