...
def helper(st):
    ...
    relevant_close_index = find_relevant_close_index(st)
    prefix, multiplier = preprocess_prefix(st[:open_index])
    st = prefix + multiplier*helper(st[open_index+1:relevant_close_index])+helper(st[relevant_close_index+1:])
    return st


def find_relevant_close_index(st):
    counter = 0
    opening_bracket_not_seen = True
    for s in st:
        if s == '[':
            counter += 1
        ...
        if not opening_bracket_not_seen and counter == 0:
            return counter

        if counter == 0 and opening_bracket_not_seen:
            opening_bracket_not_seen = False

def preprocess_prefix(st):
    prefix = ""
    counter = 0
